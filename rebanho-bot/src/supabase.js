...
async function salvarRebanho(dados, transcricao, whatsappDe) {
  const { mes, ano, fazenda = 'Grupo Ricci', categorias = [], animais = [] } = dados
  ...
  // 2. Upsert registro mensal
  const { data: mensal, error: errMensal } = await supabase
    .from('rebanho_mensal')
    .upsert(
      { mes, ano, fazenda, transcricao, whatsapp_de: whatsappDe, lote_id: loteId },
  ...
  console.log(`Salvo: ${mes}/${ano} | lote: ${dados.lote_nome || 'Geral'} | cats: ${categorias.length} | animais: ${animaisSalvos}`)
  return { id: rebanhoId, mes, ano, fazenda, loteId, totalCategorias: categorias.length, animaisSalvos }
}
